...
/*
 * Group-based scheduling extension:
 * - Threads are divided into groups A(0), B(1), C(2)
 * - Scheduler rotates between groups using defined quanta
 * - Within each group, original priority scheduling is preserved
 */
...
static thread_t *threads_selectFromGroup(int group);
...
/* new syscalls functions */
int threads_setGroup(int group) {
    thread_t *current;
    spinlock_ctx_t sc;
    ...
    current->group = group; /* Mevcut süreci yeni gruba ata */
    hal_spinlockClear(&threads_common.spinlock, &sc);

    return EOK;
}

int threads_setQuanta(int group, int quanta) {
    spinlock_ctx_t sc;

    /* Güvenlik Kontrolü: Kuantum 0 veya negatif olamaz */
    if (group < 0 || group > 2 || quanta <= 0) {
        return -EINVAL;
    }

    hal_spinlockSet(&threads_common.spinlock, &sc);
    threads_common.groupQuanta[group] = quanta;

    if (threads_common.currentGroup == group) {
        threads_common.currentQuantaLeft = quanta;
    }
    hal_spinlockClear(&threads_common.spinlock, &sc);
...
/* Group Scheduler*/
	do {
		...
		if (threads_common.currentQuantaLeft > 0) {
			selected = threads_selectFromGroup(threads_common.currentGroup);
		}

		/* Grup değiştirme vakti geldiyse veya grupta süreç yoksa */
		if (selected == NULL || threads_common.currentQuantaLeft <= 0) {
			int tries = 0;
			do {
				threads_common.currentGroup = (threads_common.currentGroup + 1) % 3;
				threads_common.currentQuantaLeft = threads_common.groupQuanta[threads_common.currentGroup];
				selected = threads_selectFromGroup(threads_common.currentGroup);
				tries++;
			} while (selected == NULL && tries < 3);
		}

		/* Hiçbir grupta süreç bulunamadıysa (Acil durum planı) */
		if (selected == NULL) {
		...
		/* Çöpe çıkmış (exit) süreçleri temizleme kontrolü (Ghost mantığı) */
		if (selected != NULL && selected->exit && !hal_cpuSupervisorMode(selected->context)) {
			selected->state = GHOST;
			LIST_ADD(&threads_common.ghosts, selected);
			_proc_threadWakeup(&threads_common.reaper);
			selected = NULL; /* Süreç çöpe gitti, yenisini aramak için başa dön */
		} else {
			break; /* Sağlam bir süreç bulduk, aramayı bitir */
		}
	} while (1);

	/* Seçilen sürecin kuantumunu düşür */
	if (selected != NULL) {
		threads_common.currentQuantaLeft--;
	}
	/* ------------------------------------------- */
...
static thread_t *threads_selectFromGroup(int group)
{
    ...
    for (i = 0; i < sizeof(threads_common.ready) / sizeof(thread_t *); i++) {
        ...
        do {
            if (t->group == group) {
                LIST_REMOVE(&threads_common.ready[i], t);
                return t;
            }
            t = t->next;
        } while (t != start);
...
/* _threads_init*/
	threads_common.currentGroup = 0;

	threads_common.groupQuanta[0] = 5; // A
	threads_common.groupQuanta[1] = 3; // B
	threads_common.groupQuanta[2] = 1; // C

	threads_common.currentQuantaLeft = threads_common.groupQuanta[0];
/* _threads_init*/
